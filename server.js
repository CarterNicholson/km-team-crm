...
// Import Companies
app.post('/api/import/companies', authenticate, (req, res) => {
  const { csv } = req.body;
  ...
  for (const row of rows) {
    ...
    try {
      db.prepare('INSERT INTO companies (name, address, city, state, created_by) VALUES (?,?,?,?,?)')
        .run(name, row['Address Line 1'] || '', row['City'] || '', row['State'] || 'WA', req.user.id);
      imported++;
    } catch (e) { errors.push(name); }
  }

  logActivity('import', 'company', null, `${imported} companies imported`, null, req.user.id);
  res.json({ imported, skipped, errors: errors.length, total: rows.length });
});

// Import Contacts
app.post('/api/import/contacts', authenticate, (req, res) => {
  const { csv } = req.body;
  if (!csv) return res.status(400).json({ error: 'No CSV data provided' });
  const rows = parseCSV(csv);
  let imported = 0, skipped = 0, errors = [];
  ...
  for (const row of rows) {
    ...
    const existing = db.prepare(
      'SELECT id FROM contacts WHERE first_name = ? AND last_name = ? AND (phone = ? OR email = ?)'
    ).get(firstName, row['Last Name'] || '', row['Phone'] || '', row['Email Address'] || '');
    if (existing) { skipped++; continue; }
    ...
    if (companyName) {
      const co = db.prepare('SELECT id FROM companies WHERE name = ?').get(companyName);
      if (co) {
        companyId = co.id;
      } else {
        const result = db.prepare('INSERT INTO companies (name, created_by) VALUES (?,?)').run(companyName, req.user.id);
  ...
  logActivity('import', 'contact', null, `${imported} contacts imported`, null, req.user.id);
  res.json({ imported, skipped, errors: errors.length, total: rows.length });
});

// Import Activities as Tasks
app.post('/api/import/activities', authenticate, (req, res) => {
  const { csv } = req.body;
  if (!csv) return res.status(400).json({ error: 'No CSV data provided' });
  const rows = parseCSV(csv);
  let imported = 0, skipped = 0, errors = [];
  ...
  for (const row of rows) {
    ...
    if (contactName) {
      ...
      const c = db.prepare('SELECT id FROM contacts WHERE first_name = ? AND last_name = ?')
        .get(parts[0] || '', parts.slice(1).join(' ') || '');
      if (c) contactId = c.id;
    }

    const isComplete = row['Is Complete'] === 'Yes' || row['Is Complete'] === '1' ? 1 : 0;
    const priority = row['Activity Type'] === 'Task' ? 'high' : 'medium';
    const dueDate = row['Date'] || row['Completed Date'] || null;

    try {
      db.prepare(`INSERT INTO tasks (title, due_date, completed, priority, contact_id, notes, created_by)
        VALUES (?,?,?,?,?,?,?)`)
        .run(subject, dueDate, isComplete, priority, contactId, row['Notes'] || '', req.user.id);
      imported++;
    } catch (e) { errors.push(subject); }
  }

  logActivity('import', 'task', null, `${imported} activities imported`, null, req.user.id);
  res.json({ imported, skipped, errors: errors.length, total: rows.length });
});

// Import Properties
app.post('/api/import/properties', authenticate, (req, res) => {
  const { csv } = req.body;
  if (!csv) return res.status(400).json({ error: 'No CSV data provided' });
  const rows = parseCSV(csv);
  let imported = 0, skipped = 0, errors = [];
  ...
  for (const row of rows) {
    ...
    const existing = db.prepare('SELECT id FROM properties WHERE address = ? AND city = ?')
      .get(address, row['City'] || '');
    if (existing) { skipped++; continue; }
    try {
      const sizeSf = parseInt((row['Size'] || '').replace(/,/g, '')) || null;
      db.prepare(`INSERT INTO properties (name, address, city, state, submarket, type, size_sf, notes, created_by)
        VALUES (?,?,?,?,?,?,?,?,?)`)
        .run(
          row['Property Name'] || '',
          address,
  ...
  logActivity('import', 'property', null, `${imported} properties imported`, null, req.user.id);
  res.json({ imported, skipped, errors: errors.length, total: rows.length });
});

// Import Deals
app.post('/api/import/deals', authenticate, (req, res) => {
  const { csv } = req.body;
  if (!csv) return res.status(400).json({ error: 'No CSV data provided' });
  const rows = parseCSV(csv);
  let imported = 0, skipped = 0, errors = [];
  ...
  for (const row of rows) {
    ...
    const existing = db.prepare('SELECT id FROM deals WHERE title = ?').get(title);
    if (existing) { skipped++; continue; }
    ...
    if (propName) {
      const p = db.prepare('SELECT id FROM properties WHERE name = ? OR address = ?').get(propName, propName);
    ...
    try {
      db.prepare(`INSERT INTO deals (title, stage, deal_type, contact_id, property_id, value, close_date, notes, assigned_to, created_by)
        VALUES (?,?,?,?,?,?,?,?,?,?)`)
        .run(title, stage, dealType, contactId, propertyId, value, closeDate, row['Notes'] || '', req.user.id, req.user.id);
      imported++;
    } catch (e) { errors.push(title); }
  }

  logActivity('import', 'deal', null, `${imported} deals imported`, null, req.user.id);
  res.json({ imported, skipped, errors: errors.length, total: rows.length });
});
